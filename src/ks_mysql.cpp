...
void MySQLClient::convert_unsupported_database_schema(Database &database) {
	...
	for (Table &table : database.tables) {
		...
		for (Column &column : table.columns) {
			...
			// mysql only has AUTO_INCREMENT, which is the same behavior (but different syntax) as the new standard SQL
			// GENERATED BY DEFAULT AS IDENTITY, so we don't need a separate default type for mysql
			if (column.default_type == DefaultType::generated_by_sequence || column.default_type == DefaultType::generated_always_as_identity) {
